...
public class dotGenerate {
    private static String[] discuss = {"现在，解决主题的问题，是非常非常重要的。 所以， ",
    ...
    private static String[] celebrityQuotes = {"伏尔泰曾经说过，不经巨大的困难，不会有伟大的事业。这不禁令我深思",
    ...
    private static String[] backGround = {"这不禁令我深思。 ",
            "带着这句话，我们还要更加慎重的审视这个问题： ",
            "这启发了我， ",
            "我希望诸位也能好好地体会这句话。 ",
            "这句话语虽然很短，但令我浮想联翩。 ",
            "这似乎解答了我的疑惑。 ",};
    private static String[] front = {"曾经说过",
            "在不经意间这样说过",
            "曾经提到过",
            "说过一句富有哲理的话"};
    private static Random random = new Random();

    private static String randomGetSentence(String[] array) {
        return array[random.nextInt(array.length)];
    }

    private static String getSomeCelebrityQuotes() {
        return randomGetSentence(celebrityQuotes)
                .replace("曾经说过", randomGetSentence(front))
                .replace("这不禁令我深思", randomGetSentence(backGround));
    }

    private static String getSomeDiscuss(String theme) {
        return randomGetSentence(discuss)
                .replace("主题", theme);
    }

    private static String processParagraph(StringBuilder stringBuilder) {
    ...
    public static String generateText(String theme) {
        ArrayList<String> sentence = new ArrayList<>();
        int totalLength = 0;
        StringBuilder paragraph = new StringBuilder();
        while (totalLength < 12000) {
            int rand = random.nextInt(100);
            if (rand < 5 && paragraph.length() > 200) {
                sentence.add(processParagraph(paragraph));
                paragraph = new StringBuilder();
            } else if (rand < 20) {
                String temp = getSomeCelebrityQuotes();
                totalLength += temp.length();
                paragraph.append(temp);
            } else {
                String temp = getSomeDiscuss(theme);
                totalLength += temp.length();
                paragraph.append(temp);
            }
        }
        return sentence.stream().reduce((item, next) -> item + "\n\n" + next).get();
    }
}
